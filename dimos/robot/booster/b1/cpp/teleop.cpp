...
constexpr auto kModePollInterval = std::chrono::milliseconds(50);
constexpr auto kWalkingModeTimeout = std::chrono::seconds(5);
...
private:
    ...
    bool switch_to_walking_locked() noexcept {
        try {
            ...
            const auto deadline = std::chrono::steady_clock::now() + kWalkingModeTimeout;
            int32_t last_get_mode_error = 0;
            while (std::chrono::steady_clock::now() < deadline) {
                booster::robot::b1::GetModeResponse response;
                last_get_mode_error = client_.GetMode(response);
                if (last_get_mode_error == 0 &&
                    response.mode_ == booster::robot::RobotMode::kWalking) {
            ...
            std::cerr << "Timed out waiting for Booster B1 walking mode";
            if (last_get_mode_error != 0) {
                std::cerr << ", last GetMode error " << last_get_mode_error;
            }
            std::cerr << std::endl;
        } catch (const std::exception& error) {
